    ...
    <div class="card"  :class="{'mobile-wrap': mobileWrap}">
      <div class="card-col">
        <div class="mobile-intro bg-white mb-3 mr-2">
          ...
          <button class="btn btn-primary mt-3" @click="handleClick(0)">{{tenantMgmt.length}} Building Blocks</button>
        </div>
        <PilarCategory :show-detail="showDetailOfCategory[0].show" :data-list="tenantMgmt" :title="'TENANT MGMT.'">
        </PilarCategory>
      </div>
      ...
      <div class="card-col">
        <div class="mobile-intro bg-white mb-3 mr-2">
          <h1>IAM</h1>
          <button class="btn btn-primary mt-3" @click="handleClick(1)">{{iam.length}} Building Blocks</button>
          <span>Tenant Management enables your organization to get a clear and governed overview of all activity going on in the cloud platforms</span>
        </div>
        <PilarCategory :show-detail="showDetailOfCategory[1].show" :data-list="iam" :title="'IAM'">
        </PilarCategory>
      </div>
      ...
      <div class="card-col">
        <div class="mobile-intro bg-white mb-3 mr-2">
          <h1>COMPLIANCE</h1>
          <button class="btn btn-primary mt-3" @click="handleClick(2)">{{compliance.length}} Building Blocks</button>
          <span>Tenant Management enables your organization to get a clear and governed overview of all activity going on in the cloud platforms</span>
        </div>
        <PilarCategory :show-detail="showDetailOfCategory[2].show" :data-list="compliance" :title="'COMPLIANCE'">
        </PilarCategory>
      </div>
      ...
      <div class="card-col">
        <div class="mobile-intro bg-white mb-3 mr-2">
          <h1>SERVICES</h1>
          <button class="btn btn-primary mt-3" @click="handleClick(4)">{{serviceEcosystem.length}} Building Blocks</button>
          <span>Tenant Management enables your organization to get a clear and governed overview of all activity going on in the cloud platforms</span>
        </div>
        <PilarCategory :show-detail="showDetailOfCategory[4].show" :data-list="serviceEcosystem" :title="'SERVICE ECOSYSTEM'">
        </PilarCategory>
      </div>
...
export default {
  ...
  methods: {
    handleClick(index): void {
      this.showDetailOfCategory[index].show = !this.showDetailOfCategory[index].show;
...
.maturity-model-landing-page {
  ...
  .card {
    ...
    h1 {font-weight: 800; color: white}
    .card-col {
      flex-grow: 1;
      max-width: 20%;
      ...
      @media only screen and (max-width: 580px) {
        ...
        .mobile-intro {
          ...
          h1 {margin: 8px 0; color: #9fd9ec}
          button {background: #9fd9ec; border-color: #9fd9ec}
          span {color: #4a4a4a
          }
        }
      }
